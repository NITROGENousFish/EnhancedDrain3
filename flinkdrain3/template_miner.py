...
from flinkdrain3.drain import Drain, LogCluster 
# from flinkflinkdrain3.jaccard_drain import JaccardDrain
from flinkdrain3.masking import LogMasker
from flinkdrain3.persistence_handler import PersistenceHandler
...
config_filename = 'drain3-default.json'
...
class TemplateMiner:
    ...
    def load_state(self):
        ...
        logger.info(f"Restored {len(loaded_drain.clusters)} clusters "
                    f"built from {loaded_drain.total_cluster_size} messages")
    ...
    def save_state(self, snapshot_reason):
        ...
        logger.info(f"Saving state of {len(self.drain.clusters)} clusters "
                    f"with {self.drain.total_cluster_size} messages, {len(state)} bytes, "
