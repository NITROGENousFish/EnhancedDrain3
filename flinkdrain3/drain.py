...
from typing import List, Dict, Sequence, Union, Tuple,Callable
from abc import abstractmethod

from cachetools import LRUCache, Cache

import flinkdrain3.profiler as p
...
class DrainBase:
    def __init__(self,
        ...
        :param extra_delimiters: delimiters to apply when splitting log message into words (in addition to whitespace). #* 除了空格额外的分隔符
    ...
    @abstractmethod
    def get_seq_distance(self,token_list1:list,token_list2:list,include_params:bool) -> (float,int):
        ...
    
    @abstractmethod
    def tree_search(self,root_node:Node,tokens:list,sim_th:float,include_params:bool) -> Union[None, LogCluster]:
        ...
    ...
    @abstractmethod
    def create_template(self,token_list1:list,token_list2:list) -> List:
        ...
    
    def split_log_as_tokens(self, content:str) -> List:
    ...
    def fast_match(self, cluster_ids: Sequence, tokens: list, sim_th: float, include_params: bool) -> Union[None, LogCluster]:
        """
    ...
    def add_log_message(self, content: str):
        ...
        if match_cluster is None:
            ...
            def _create_cluster():
                ...
                return update_type, match_cluster
            update_type,match_cluster = _create_cluster()
        # Add the new log message to the existing cluster
        else:
            @p.PROFILER('cluster_exist')
            def _cluster_exist():
    ...
    def get_clusters_ids_for_seq_len(self, seq_fir):
        ...
        def _append_clusters_recursive(node: Node, id_list_to_fill: list):
            id_list_to_fill.extend(node.cluster_ids)
            for child_node in node.key_to_child_node.values():
                _append_clusters_recursive(child_node, id_list_to_fill)
        ...
        target = []
        _append_clusters_recursive(cur_node, target)
        return target


class Drain(DrainBase):
    @p.PROFILER("tree_search")
    ...
    def match(self, content: str, full_search_strategy="never") -> Union[None, Callable]:
        """
